...
pub struct GameConfig {
    ...
    /// Whether the game ships the boot loader art (loading progress bar + step-2 feature
    /// panel). Defaults on — every slot has a two-step loading screen.
    #[serde(default = "default_true")]
    pub has_loader: bool,

    /// Whether the game ships bitmap fonts (BMFont atlases for runtime win amounts / labels).
    #[serde(default)]
    pub has_fonts: bool,
    /// Bitmap-font definitions, baked deterministically to BMFont `.xml` + `.webp` at export.
    #[serde(default)]
...
/// The default cue list seeded when a project first enables audio — the "Core SFX +
/// base music" set that covers ~90% of a slot's perceived audio polish.
pub fn default_audio_cues() -> Vec<AudioCueDef> {
    use AudioKind::{Music, Sfx};
    let cue = |key: &str, kind, name: &str, looped, gain: f32, secs: f32, desc: &str| AudioCueDef {
        key: key.into(),
        kind,
        name: name.into(),
    ...
    // Names follow the Stake Engine web-sdk convention: `bgm_*` looping music, `sfx_*` effects,
    // `jng_*` jingles/stings. These become the audiosprite cue names + the game's SoundName union.
    vec![
        cue("bgm_main", Music, "Base-game music", true, 0.7, 30.0, "the main ambient background music bed, seamless loop"),
        cue("sfx_btn_spin", Sfx, "Spin button", false, 1.0, 0.3, "a short tactile click/whoosh when the spin button is pressed"),
        cue("sfx_reel_spin", Sfx, "Reel spin", true, 0.6, 1.0, "a continuous whir while the reels are spinning, seamless loop"),
        cue("sfx_reel_stop_1", Sfx, "Reel stop", false, 1.0, 0.2, "a tight thunk/click as a reel lands"),
        cue("sfx_anticipation", Sfx, "Anticipation", true, 0.9, 2.0, "a rising tension build when scatter/bonus symbols are landing"),
        cue("sfx_win_tick", Sfx, "Win rollup", true, 0.8, 0.15, "a repeating coin/counter tick during the win count-up, loops"),
        cue("sfx_winlevel_small", Sfx, "Small win", false, 1.0, 1.5, "a short cheerful chime for a small win"),
        cue("sfx_winlevel_big", Sfx, "Big win", false, 1.0, 3.0, "an escalating celebratory fanfare for a big win"),
        cue("sfx_winlevel_mega", Sfx, "Mega win", false, 1.0, 4.0, "an over-the-top triumphant fanfare for a mega win"),
        cue("jng_bonus", Sfx, "Scatter / bonus", false, 1.0, 2.0, "a magical celebratory sting when a bonus/scatter triggers"),
        cue("sfx_btn_general", Sfx, "UI click", false, 0.8, 0.15, "a crisp, quiet UI button click"),
        cue("sfx_coin", Sfx, "Coin / collect", false, 1.0, 1.0, "a bright coin/cash payout collect sound"),
    ]
}

// ── Bitmap fonts ────────────────────────────────────────────────────────────────

/// One bitmap font the game needs — a bundled typeface rasterized into a colored BMFont
/// atlas (`.xml` + `.webp` page) for PIXI `BitmapText`. Deterministic (no AI), baked at
/// export. Config-driven asset def, like `AudioCueDef`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize, Type)]
#[serde(rename_all = "camelCase")]
pub struct FontDef {
    /// Stable key = asset key + BMFont `face`, e.g. `font_gold`, `font_white`.
    pub key: String,
    #[serde(default)]
    pub name: String,
    /// Bundled typeface id (see `list_font_typefaces`), e.g. "luckiest_guy" | "titan_one".
    #[serde(default = "d_typeface")]
    pub typeface: String,
    /// Glyph cap size in px (drives atlas resolution).
    #[serde(default = "d_font_size")]
    pub size_px: u32,
    /// Vertical gradient fill top→bottom (equal = solid). Hex `#rrggbb`.
    #[serde(default = "d_fill")]
    pub fill_top: String,
    #[serde(default = "d_fill")]
    pub fill_bottom: String,
    /// Outline color (hex) — drawn when `outline_px > 0`.
    #[serde(default = "d_outline_color")]
    pub outline_color: String,
    /// Outline width in px (0 = none).
    #[serde(default = "d_outline_px")]
    pub outline_px: u32,
}

fn d_typeface() -> String {
    "luckiest_guy".into()
}
fn d_font_size() -> u32 {
    96
...
/// The default font set seeded when a project first enables fonts: a clean white numeral
/// font + a gold gradient one (win amounts).
pub fn default_fonts() -> Vec<FontDef> {
    let f = |key: &str, name: &str, face: &str, size: u32, top: &str, bot: &str, oc: &str, ow: u32| FontDef {
        key: key.into(),
        name: name.into(),
        typeface: face.into(),
        size_px: size,
        fill_top: top.into(),
        fill_bottom: bot.into(),
        outline_color: oc.into(),
        outline_px: ow,
    };
    vec![
        f("font_white", "White numerals", "titan_one", 96, "#ffffff", "#e6ecf5", "#1a2230", 6),
        f("font_gold", "Gold win", "luckiest_guy", 108, "#ffe89a", "#e0a13a", "#4a2d0a", 7),
    ]
}
